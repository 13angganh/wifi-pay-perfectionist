...
const inputStyle: React.CSSProperties = {
  ...
  color:'var(--txt)', padding:'10px 14px', borderRadius:'var(--r-sm)', fontSize:20,
  letterSpacing:8, textAlign:'center', outline:'none', marginBottom:10,
...
function Btn({ label, onClick, danger=false, secondary=false, icon }: {
  label: string; onClick: () => void; danger?: boolean; secondary?: boolean; icon?: React.ReactNode;
}) {
  return (
...
function PinCard({ title, desc, pinErr, children }: {
  title: string; desc?: string; pinErr?: string; children: React.ReactNode;
}) {
  return (
...
export default function SettingsPinSection() {
  ...
  return (
    <>
      ...
      {pinStep === 'enable-new' && (
        <PinCard pinErr={pinErr} title={t('settings.pin.newTitle')} desc={t('settings.pin.enterNew')}>
      ...
      {pinStep === 'enable-confirm' && (
        <PinCard pinErr={pinErr} title={t('pin.confirm')} desc={t('settings.pin.reenterNew')}>
      ...
      {pinStep === 'disable-verify' && (
        <PinCard pinErr={pinErr} title={t('settings.pinDisable')} desc={t('settings.pin.enterCurrent')}>
          <PinInput value={pin1} onChange={setPin1} />
      ...
      {pinStep === 'change-old' && (
        <PinCard pinErr={pinErr} title={t('settings.pinChange')} desc={t('settings.pin.enterOld')}>
          <PinInput value={pin1} onChange={setPin1} />
