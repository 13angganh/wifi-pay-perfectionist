...
// v11.3: Tambah Link Google + tampilkan provider aktif
'use client';
...
export default function AccountModal({ open, onClose }: Props) {
  ...
  // Ambil providers saat modal terbuka atau setelah link berhasil (refreshKey = cache bust)
  ...
  async function handleLinkGoogle() {
    ...
    if (res.error) {
      ...
      showToast('Google berhasil dihubungkan ke akun ini ✓');
      setRefreshKey(k => k + 1);
    }
  }
  ...
  return (
    <div className="modal-bg" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:8, padding:12, marginBottom:14 }}>
          ...
          <div style={{ display:'flex', gap:6, marginTop:10, flexWrap:'wrap' }}>
            {providers.includes('password') && (
              <span style={{ fontSize:9, fontWeight:700, letterSpacing:'.05em', padding:'2px 8px', borderRadius:100, background:'rgba(59,130,246,0.12)', color:'#60a5fa', border:'1px solid rgba(59,130,246,0.25)' }}>
                ✉ EMAIL
              </span>
            )}
            {isGoogleLinked && (
              <span style={{ fontSize:9, fontWeight:700, letterSpacing:'.05em', padding:'2px 8px', borderRadius:100, background:'rgba(234,67,53,0.1)', color:'#f87171', border:'1px solid rgba(234,67,53,0.25)' }}>
                G GOOGLE
              </span>
            )}
          </div>
        ...
        {/* Hubungkan Google — tampil jika belum terhubung */}
        {!isGoogleLinked && (
          <button
            style={{ ...btnBase, marginBottom:12, opacity: linking ? 0.6 : 1, pointerEvents: linking ? 'none' : 'auto' }}
        ...
        {isGoogleLinked && (
          <div style={{ fontSize:11, color:'var(--c-lunas)', textAlign:'center', marginBottom:12, padding:'8px 12px', background:'rgba(34,197,94,0.07)', borderRadius:'var(--r-sm)', border:'1px solid rgba(34,197,94,0.2)' }}>
            ✓ Google sudah terhubung — bisa login via Google atau Email
          </div>
        )}

        <button className="lf-btn secondary" style={{ marginBottom:8 }} onClick={handleSwitch}>
          ↔ Ganti Akun
        </button>
        <button className="lf-btn" style={{ background:'rgba(239,68,68,0.08)', color:'var(--c-belum)', border:'1px solid rgba(239,68,68,0.25)' }} onClick={handleLogout}>
          Keluar
        </button>
      </div>
    </div>
